class Patient():
    ...
    def __init__(self, ED, ESI):
        ...
        if self.ED.chance or np.random.uniform(0,1) <= ESIdict[self.ESI][0]:
            self.needs.add("labs")
        if self.ED.chance or np.random.uniform(0,1) <= ESIdict[self.ESI][1]:
            self.needs.add("rads")
        if self.ED.chance or np.random.uniform(0,1) <= ESIdict[self.ESI][2]:
